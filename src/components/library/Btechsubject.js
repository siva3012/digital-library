...
<div style={{textAlign:"center"}} className="mt-5 search"><input type="text" placeholder="Search for the book" id="myInput" className="searchbar" onKeyUp={filterFunction}></input>
  ...
  <div className="text-center mt-5 ml-5 buttons">{array.map((branch)=>(<button value={branch.book} id={branch.book} className="buttontype btn" ><div className='name'><span className="span1">name of the book</span><br></br><span id="p1">{branch.book}</span></div><div className="name"><span className="span1">Author</span><br></br><span>{branch.author}</span></div><div className="name"><span className="span1">Subject</span><br></br><span>{branch.subject}</span></div><div className="name"><span className="span1">website</span><br></br><span>{branch.website}</span></div><div className="lower"><img src={Likes} onClick={getid}></img><span>{branch.likes}</span><span className="lowerimg2"><img src={Save} alt={Save}></img></span></div></button>))}</div>
</div>
